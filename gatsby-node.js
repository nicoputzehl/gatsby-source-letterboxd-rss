...
exports.sourceNodes = async (
  ...
  if (!letterboxdUserName)
    return reporter.panic(
      "gatsby-source-letterboxd-rss: You must provide a letterboxd username"
    )
  ...
  const diaryData = await getDiaryData(letterboxdUserName)

  const processLetterboxdData = async (itemData, id) => ({
    ...itemData,
    id: createNodeId(id),
    internal: {
      type: `LetterboxdData`,
  ...
  await Promise.all(
    diaryData.map(async (diaryEntry, index) => {
      return createNode(await processLetterboxdData(diaryEntry, index))
    })
  )
}
